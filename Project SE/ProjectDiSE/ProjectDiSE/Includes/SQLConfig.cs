...
namespace ProjectDiSE.Includes
{
    class SQLConfig
    {
        private MySqlConnection con = new MySqlConnection("server=localhost;user id=root;database=db_stock;sslMode=none");
        ...
        {
            Execute_Query("UPDATE `genid` SET `END`=`END`+`INCREMENT` WHERE `DESCRIPTION`='" + id + "'");
        }
    }
}
